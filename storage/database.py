...
class FeedbackModel(Base):
    """Model for user feedback on alerts and keyword matches"""
    __tablename__ = 'feedback'
    
    id = Column(Integer, primary_key=True)
    alert_id = Column(Integer, ForeignKey('alerts.id', ondelete='SET NULL'))
    match_id = Column(Integer, ForeignKey('keyword_matches.id', ondelete='SET NULL'))
    
    # Feedback type
    feedback_type = Column(String(20), nullable=False)  # 'true_positive', 'false_positive', 'false_negative'
    
    # Context about what was flagged
    ...
    # Who gave feedback
    feedback_from = Column(String(100), default='dashboard_user')
    ...
    __table_args__ = (
        ...
        Index('idx_feedback_keyword', 'keyword'),
    )
...
class DatabaseManager:
    ...
    def to_local_time(utc_dt):
        """Convert UTC datetime to India local time"""
        if utc_dt is None:
            return None
        return utc_dt.replace(tzinfo=timezone.utc).astimezone(INDIA_TZ)
    ...
    def save_feedback(self, feedback_data: Dict[str, Any]) -> int:
        ...
        with self.db.get_session() as session:
            feedback = FeedbackModel(
                ...
                keyword=feedback_data.get('keyword'),
                category=feedback_data.get('category'),
                original_confidence=feedback_data.get('original_confidence'),
                adjusted_confidence=feedback_data.get('adjusted_confidence'),
                source_url=feedback_data.get('source_url'),
                feedback_from=feedback_data.get('feedback_from', 'dashboard_user'),
                feedback_comment=feedback_data.get('feedback_comment', '')
            )
            session.add(feedback)
            session.flush()
            
            # Also update the keyword match record if it exists
            if feedback_data.get('match_id'):
                match = session.query(KeywordMatchModel).filter(
                    KeywordMatchModel.id == feedback_data.get('match_id')
                ).first()
                if match:
                    # Store feedback in match record for quick access
                    match.feedback_type = feedback_data.get('feedback_type')
            
            self.logger.info(f"Saved feedback (ID: {feedback.id}, Type: {feedback_data.get('feedback_type')})")
            return feedback.id

    def get_feedback_stats(self, keyword: str = None, category: str = None) -> Dict[str, Any]:
        """Get feedback statistics for confidence adjustment"""
        with self.db.get_session() as session:
            query = session.query(FeedbackModel)
            
            if keyword:
                query = query.filter(FeedbackModel.keyword == keyword)
            if category:
            ...
            fn_count = sum(1 for f in feedbacks if f.feedback_type == 'false_negative')
            ...
            if total > 0:
                ...
                return {
                    ...
                    'precision': precision,
                    'adjusted_confidence': precision * 0.9  # Scale to 0-0.9 range
                }
