...
class IntentClassifier:
    """Intent classifier using fine-tuned BART model"""
    
    def __init__(self):
        self.logger = logging.getLogger(__name__)
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        # Path to fine-tuned model (try both possible names)
        model_paths = ["./bart_intent_model_best", "./bart_intent_model_final"]
        model_loaded = False
        ...
        for model_path in model_paths:
            try:
                ...
                self.logger.info(f"[SUCCESS] Loaded fine-tuned BART model from {model_path}")
                break
        ...
        if not model_loaded:
            self.logger.warning("Fine-tuned model not found. Using fallback.")
            self.use_finetuned = False
            self.intent_categories = [
                "credential_dump", "data_breach_announcement", "proprietary_data_sale",
                "agriculture_research", "ransomware_threat", "marketplace_listing",
                "technical_discussion", "false_positive_sample"
            ]
            return
        
        # ============================================================
        # FIX 1: Handle id2label mapping correctly
        # ============================================================
        # The model may have id2label as dict with string keys or int keys
        if hasattr(self.model.config, 'id2label'):
            ...
            self.intent_categories = list(self.id2label.values())
        else:
            # Fallback categories
            self.intent_categories = [
                "credential_dump", "data_breach_announcement", "proprietary_data_sale",
            ...
            self.id2label = {i: label for i, label in enumerate(self.intent_categories)}
            self.label2id = {label: i for i, label in enumerate(self.intent_categories)}
        
        self.logger.info(f"Loaded model with {len(self.intent_categories)} intent categories")
        self.logger.info(f"Categories: {self.intent_categories}")
    
    def classify(self, text: str, max_length: int = 256):
        """Classify intent using fine-tuned model"""
        
        if self.use_finetuned:
            return self._finetuned_classify(text, max_length)
        else:
            return self._rule_based_classify(text)
    
    def _finetuned_classify(self, text: str, max_length: int = 256):
        """Use fine-tuned BART model"""
        ...
        # ============================================================
        # Override based on keyword detection (fixes bias)
        # ============================================================
        text_lower = text.lower()
        
        # Ransomware detection
        if any(kw in text_lower for kw in ['ransom', 'encrypted', 'decryption', 'pay btc', 'bitcoin']):
            predicted_class = self.label2id.get('ransomware_threat', predicted_class)
            # Boost confidence for ransomware
            confidence = max(confidence, 0.65)
            self.logger.debug(f"Ransomware keywords detected - overriding to ransomware_threat (conf: {confidence:.2f})")
        
        # Marketplace listing detection
        elif any(kw in text_lower for kw in ['for sale', 'selling', 'price:', 'btc', 'access to']):
            predicted_class = self.label2id.get('marketplace_listing', predicted_class)
            # Moderate confidence for marketplace
            confidence = max(confidence, 0.55)
            self.logger.debug(f"Marketplace keywords detected - overriding to marketplace_listing (conf: {confidence:.2f})")
        
        # Data breach announcement detection
        elif any(kw in text_lower for kw in ['breach', 'hacked', 'compromised', 'stolen', 'breach alert']):
            predicted_class = self.label2id.get('data_breach_announcement', predicted_class)
            confidence = max(confidence, 0.65)
            self.logger.debug(f"Breach keywords detected - overriding to data_breach_announcement (conf: {confidence:.2f})")
        
        # Technical discussion (safe content)
        elif any(kw in text_lower for kw in ['how to', 'configure', 'question about', 'discussion']):
            predicted_class = self.label2id.get('technical_discussion', predicted_class)
            confidence = min(confidence, 0.50)  # Lower confidence for safe content
            self.logger.debug(f"Technical discussion detected - setting to technical_discussion (conf: {confidence:.2f})")
        
        # Get all scores
        all_scores = {}
        for i in range(len(self.intent_categories)):
            if i in self.id2label:
                label = self.id2label[i]
            elif str(i) in self.id2label:
                label = self.id2label[str(i)]
            else:
                label = self.intent_categories[i] if i < len(self.intent_categories) else f"class_{i}"
            
            if i < probs.shape[1]:
                all_scores[label] = probs[0][i].item()
            else:
                all_scores[label] = 0.0
        
        # Get intent name
        if predicted_class in self.id2label:
            intent = self.id2label[predicted_class]
        elif str(predicted_class) in self.id2label:
            intent = self.id2label[str(predicted_class)]
        else:
            intent = self.intent_categories[predicted_class] if predicted_class < len(self.intent_categories) else "technical_discussion"
        
        self.logger.debug(f"Intent: {intent} (conf: {confidence:.2f})")
        
        # Import here to avoid circular imports
        from nlp_analysis.analyzer import IntentClassification
    ...
    def _rule_based_classify(self, text: str):
        ...
        if 'password:' in text_lower or 'api_key' in text_lower:
            return IntentClassification(
                intent="credential_dump",
                confidence=0.75,
                all_scores={"credential_dump": 0.75}
            )
        elif 'selling' in text_lower or 'for sale' in text_lower or 'btc' in text_lower:
            return IntentClassification(
                intent="marketplace_listing",
                confidence=0.70,
                all_scores={"marketplace_listing": 0.70}
            )
        elif 'sample' in text_lower or 'test' in text_lower or 'demo' in text_lower:
            return IntentClassification(
                intent="false_positive_sample",
                confidence=0.80,
                all_scores={"false_positive_sample": 0.80}
            )
        elif 'breach' in text_lower or 'hacked' in text_lower or 'compromised' in text_lower:
            return IntentClassification(
                intent="data_breach_announcement",
                confidence=0.70,
                all_scores={"data_breach_announcement": 0.70}
            )
        elif 'ransom' in text_lower or 'encrypted' in text_lower or 'bitcoin' in text_lower:
            return IntentClassification(
                intent="ransomware_threat",
                confidence=0.70,
                all_scores={"ransomware_threat": 0.70}
            )
        else:
            return IntentClassification(
                intent="technical_discussion",
                confidence=0.50,
                all_scores={"technical_discussion": 0.50}
            )
    
    def calculate_threat_score(self, intent_result, entities: list = None, has_keyword_matches: bool = True) -> float:
        """Calculate threat score from intent classification"""
        ...
        threat_levels = {
            "credential_dump": 90,
            "data_breach_announcement": 80,
            "proprietary_data_sale": 75,
            "ransomware_threat": 80,
            "marketplace_listing": 60,
            "agriculture_research": 20,
            "technical_discussion": 15,
            "false_positive_sample": 5
        }
        
        base_score = threat_levels.get(intent, 50)
        
        # ============================================================
        # CRITICAL FIX: If no keyword matches, reduce threat score
        # ============================================================
        if not has_keyword_matches:
            # Reduce score by 50-70% depending on intent
            if intent in ["credential_dump", "data_breach_announcement"]:
                base_score = base_score * 0.3  # 70% reduction
            elif intent in ["ransomware_threat", "proprietary_data_sale"]:
                base_score = base_score * 0.4  # 60% reduction
            else:
                base_score = base_score * 0.5  # 50% reduction
        ...
        # Boost based on entities (if provided)
        if entities:
            credential_entities = [e for e in entities if hasattr(e, 'label') and e.label == 'CREDENTIAL']
            if credential_entities:
...
class NLPAnalyzer:
    ...
    def analyze_document(self, document: ProcessedDocument, 
        ...
        has_keyword_matches = keyword_matches is not None and len(keyword_matches) > 0
        threat_score = self.intent_classifier.calculate_threat_score(intent, entities, has_keyword_matches)
