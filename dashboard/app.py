...
def get_threat_timeline():
    """Get threat score timeline for charts"""
    hours = request.args.get('hours', 168, type=int)
    
    timeline = storage.get_threat_timeline(hours)
...
def chart_alert_distribution():
    """Generate alert distribution pie chart"""
    with db_manager.get_session() as session:
        from storage.database import AlertModel
        ...
        fig = go.Figure(data=[go.Pie(
            ...
            hole=0.4,
            marker=dict(colors=['#dc3545', '#fd7e14', '#ffc107', '#17a2b8', '#6c757d'])
        )])
