...
class EntityExtractor:
    ...
    def extract_credentials(self, text: str, source_url: str = "") -> List[ExtractedEntity]:
        ...
        Returns:
            List of extracted credential entities
        """
        credentials = []
        ...
        for cred_type, pattern in self.credential_patterns.items():
            for match in pattern.finditer(text):
                ...
                redacted_value = credential[:10] + "..." if len(credential) > 10 else credential # (Don't store full passwords).
...
class DocumentProcessor:
    ...
    def remove_duplicate_credentials(self, credentials):
        unique = {}
        
        for cred in credentials:
            if cred.value not in unique:
                unique[cred.value] = cred
